...
then
  ...
  sudo gcloud compute ssh $VM_NAME --zone $ZONE_NAME --internal-ip --command "mkdir github; cd github; git clone https://github.com/Tulsishah/gcsfuse-tulsishah7.git; cd gcsfuse; git checkout master;"
